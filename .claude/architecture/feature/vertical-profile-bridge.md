...
```
Guarantees: (a) evidence files first — `{artifact_dir}/{call_sid}.transcript.txt` and `{call_sid}.analysis.json` (= `analysis.raw`) are written before any network side effect; `{call_sid}.brief.md` is written once, LAST, after SMS and email have been attempted, so it can state their outcome (sid or error) — the email body is the same brief text minus the email-outcome line; (b) SMS sent iff `analysis.outcome in profile.sms` AND `record.caller_number` non-empty AND no `{artifact_dir}/{call_sid}.sms-sent` marker exists; marker written *before* the send call — at-most-once: a crash between send and marker can never produce a second SMS; a send failure after the marker leaves that call with zero SMS, which is recorded in `DispatchReport.errors` AND in the brief/email so the operator can send it by hand. **RESOLVED 2026-09-12 (owner-ratified, `dispatch` slice plan Q4): keep at-most-once — mark before send.** This favours A2's "not two" over its "not zero"; the rejected alternative (mark after send) flips that. A duplicate SMS to a prospect cannot be recalled; a missed one is caught by the brief and the operator email, both of which state the SMS outcome. Recorded in `.claude/overseer/escalations.md`, 2026-09-12 PRODUCT_DECISION; body = `template.text.format(**template.links)`; (c) email always attempted to `profile.operator_email`, subject `[{profile.display_name}] {outcome} — {caller_number}` (was `profile.name`; amended by profile-loader Q12); (d) **each of the FIVE effects — the transcript write, the analysis-json write, SMS, email, and the brief write — is wrapped independently; failures land in `errors`, `dispatch` never raises** (amended 2026-09-12, owner-ratified, by the `dispatch` slice plan Q10-A; previously scoped to SMS/email only, which was silent on the file writes. The silence was read as a ceiling by one reading and a floor by another, and the readings differ materially: under the ceiling a failed transcript write aborts `dispatch` BEFORE the email, so a real finished call reaches the operator as silence. Ordering is unchanged — evidence files are still attempted before any network effect, which is what guarantee (a) requires); (e) `post_call` catches and logs every exception itself, so under normal operation S3's guarantee (b) — log and continue if `on_call_end` raises — is a second line of defence that never fires.
...
**Sequencing requirement (added 2026-09-12 by the `dispatch` slice plan — these artifacts are DESTROYED mid-S7 if ignored): 7a's read-out must be complete, or the artifacts copied off the instance, BEFORE 7b's first redeploy.** S5 writes these files to the instance's local disk; the service runs `min-instances=1`/`max-instances=1`, so 7b's two redeploys replace the instance and take 7a's evidence with them. Cloud/DB artifact storage is deferred (`dispatch` slice plan, Phase 5), which is what leaves this a sequencing obligation rather than a non-issue.
...
| 7b | **A4 profile switch** | **GATE — ADR-0001 (caller data residency) must be RATIFIED by the owner before ANY real call, including 7a's first. See `docs/adr/0001-caller-data-residency.md`. A real call is when un-region-controlled caller data starts existing, and it cannot be un-created. Deploy, wiring and smoke tiers 1-2 are NOT gated.** Then: **FIRST complete 7a's read-out (or copy `{call_sid}.transcript.txt` / `.brief.md` / `.jsonl` off the instance) — the redeploy below destroys them; see Edge S7.** Then: owner redeploys the SAME Cloud Run service and SAME Twilio number with `DECANA_PROFILE=eco-consultant` (env-only change, no image rebuild); one call: hears the eco disclosure, gets the eco SMS/email; `git diff --stat` between the broker and eco states shows `profiles/` only. Owner then redeploys back to `mortgage-broker`. **Owner decision, batched (recommended: reuse the same number):** reusing the number means the real line briefly runs the eco profile — fine while only test callers know the number; provisioning a second number is another hard-to-undo act (MVP doc) and buys nothing for a throwaway profile. | **Yes** — two redeploys + one call. |
...
- Any step: a product/threshold question Phase 1 did not answer → batched to the owner. Currently batched: ~~(1) SMS marker order~~ — **RESOLVED 2026-09-12, owner ratified at-most-once (mark before send)**; (2) A4 on the same Twilio number vs a second number (recommended same) — still open, belongs to S7.
