...
FIXTURE = (
    ...
    TranscriptTurn(
        role="model", text="Are you looking to borrow more, or keep the same balance?"
    ),
    TranscriptTurn(role="caller", text="Same balance. What rate could I get?"),
    TranscriptTurn(
        role="model", text="I can't quote rates -- an adviser will follow up."
    ),
)
...
def main() -> int:
    ...
    if result.outcome == UNCLASSIFIED:
        ...
        failures.append(
            f"the model returned unclassified on a clean fixture: {result.summary}"
        )
    if not result.raw:
